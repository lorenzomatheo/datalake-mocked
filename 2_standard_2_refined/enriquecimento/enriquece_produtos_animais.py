...
# MAGIC
# MAGIC Este notebook enriquece produtos da categoria **Produtos para Animais**.
...
from maggulake.pipelines.schemas.enriquece_produtos_animais import (
...
SUPER_CATEGORIA = "Produtos para Animais"
TABLE_OUTPUT = Table.extract_product_info_produtos_animais

# COMMAND ----------

env = DatabricksEnvironmentBuilder.build(
    "gpt_enriquece_produtos_animais",
    dbutils,
...
env.create_table_if_not_exists(TABLE_OUTPUT, schema_enriquece_produtos_animais)
...
if llm_config.provider == "gemini_vertex":
    CONTEXT_CACHE_NAME = create_vertex_context_cache(
        ...
        system_instruction=PROMPT_ENRIQUECIMENTO_PRODUTOS_ANIMAIS_SYSTEM,
...
produtos_pendentes = filtra_notifica_produtos_enriquecimento(
    ...
    days_back=DAYS_BACK,
    script_name="gpt_enriquece_produtos_animais",
)
...
llm_caller = TaggingProcessor(
    ...
    location=llm_config.location,
    model=llm_config.model,
    prompt_template=PROMPT_ENRIQUECIMENTO_PRODUTOS_ANIMAIS_USER_TEMPLATE,
    output_schema=EnriquecimentoProdutosAnimais,
    temperature=0.1,
...
SCHEMA_FIELDS = [field.name for field in enriquece_produtos_animais_schema.fields]
