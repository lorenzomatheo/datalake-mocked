...
# MAGIC
# MAGIC Este notebook enriquece produtos da categoria **Produtos para Casa**.
...
from maggulake.pipelines.schemas.enriquece_produtos_casa import (
...
SUPER_CATEGORIA = "Produtos para Casa"
TABLE_OUTPUT = Table.extract_product_info_produtos_casa

# COMMAND ----------

env = DatabricksEnvironmentBuilder.build(
    "gpt_enriquece_produtos_casa",
    dbutils,
...
env.create_table_if_not_exists(TABLE_OUTPUT, schema_enriquece_produtos_casa)
...
if llm_config.provider == "gemini_vertex":
    CONTEXT_CACHE_NAME = create_vertex_context_cache(
        ...
        system_instruction=PROMPT_ENRIQUECIMENTO_PRODUTOS_CASA_SYSTEM,
...
produtos_pendentes = filtra_notifica_produtos_enriquecimento(
    ...
    days_back=DAYS_BACK,
    script_name="gpt_enriquece_produtos_casa",
)
...
llm_caller = TaggingProcessor(
    ...
    location=llm_config.location,
    model=llm_config.model,
    prompt_template=PROMPT_ENRIQUECIMENTO_PRODUTOS_CASA_USER_TEMPLATE,
    output_schema=EnriquecimentoProdutosCasa,
    temperature=0.1,
...
SCHEMA_FIELDS = [field.name for field in enriquece_produtos_casa_schema.fields]
