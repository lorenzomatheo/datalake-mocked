...
# MAGIC
# MAGIC Este notebook enriquece produtos da categoria **Materiais para Saúde**.
...
from maggulake.pipelines.schemas.enriquece_materiais_saude import (
...
SUPER_CATEGORIA = "Materiais para Saúde"
TABLE_OUTPUT = Table.extract_product_info_materiais_saude

# COMMAND ----------

env = DatabricksEnvironmentBuilder.build(
    "gpt_enriquece_materiais_saude",
    dbutils,
...
env.create_table_if_not_exists(TABLE_OUTPUT, schema_enriquece_materiais_saude)
...
if llm_config.provider == "gemini_vertex":
    CONTEXT_CACHE_NAME = create_vertex_context_cache(
        ...
        system_instruction=PROMPT_ENRIQUECIMENTO_MATERIAIS_SAUDE_SYSTEM,
...
produtos_pendentes = filtra_notifica_produtos_enriquecimento(
    ...
    days_back=DAYS_BACK,
    script_name="gpt_enriquece_materiais_saude",
)
...
llm_caller = TaggingProcessor(
    ...
    location=llm_config.location,
    model=llm_config.model,
    prompt_template=PROMPT_ENRIQUECIMENTO_MATERIAIS_SAUDE_USER_TEMPLATE,
    output_schema=EnriquecimentoMateriaisSaude,
    temperature=0.1,
...
SCHEMA_FIELDS = [field.name for field in enriquece_materiais_saude_schema.fields]
